...
export default function Workflow() {
  return (
    <div id="workflow" className="bg-white py-24 sm:py-32 border-y border-zinc-200">
            ...
            <div className="space-y-12 md:space-y-0">
              {steps.map((step, index) => (
                <div key={step.id} className={`relative flex flex-col md:flex-row gap-8 items-center ${index % 2 === 0 ? 'md:flex-row-reverse' : ''} md:py-8`}>
                  <div className="hidden sm:flex absolute left-6 md:left-1/2 -translate-x-1/2 w-12 h-12 rounded-full bg-indigo-100 border-4 border-white items-center justify-center text-xl shadow-sm z-10">
                    {step.icon}
                  </div>

                  <div className={`md:w-1/2 ${index % 2 === 0 ? 'md:pl-16 lg:pl-24' : 'md:pr-16 lg:pr-24 text-left md:text-right'}`}>
                    <div className="flex flex-col gap-2 bg-zinc-50 p-6 rounded-2xl border border-zinc-200 shadow-sm hover:shadow-md transition-shadow">
